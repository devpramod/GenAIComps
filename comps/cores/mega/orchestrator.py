    ...
    async def execute(
        ...
        **kwargs,
        ...
        if is_llm_vlm and llm_parameters.stream:
            # Start metrics tracking for streaming LLM requests
            self.conversation_metrics.start_request()
            ...
            # Start metrics tracking for non-streaming LLM requests
            if is_llm_vlm:
                self.conversation_metrics.start_request()
            ...
            if response.content_type == "audio/wav":
                ...
                if is_llm_vlm and "choices" in data and data["choices"]:
                    ...
                    # Calculate and add metrics to the response
                    input_text = json.dumps(input_data.get('messages', ''))
                    metrics = self.conversation_metrics.calculate_metrics(input_text)
